...
END_MSG
    ...
    elsif ($file =~ /\.(b?gz|bz2|zip|Z|[lxX]z)$/) {
        if ($mode eq 'r') {
            croak "File ($file) not found" unless (-e $file);
            croak "File ($file) was not readable" unless (-r $file);
            # assume we have zcat from http://www.nongnu.org/zutils/zutils.html
            # which can handle all of these formats
            if ( ! open $fh, "zcat $file |") {
                my $prog = $1 eq 'lz' ? 'plzip'
                         : $1 eq 'bz2' ? 'bzip2'
                         : ($GzipIn || $Gzip);
            ...
            my $prog = ($1 eq 'gz' || $1 eq 'bgz') ? ($GzipOut || $Gzip)
                     : $1 eq 'bz2' ? 'bzip2'
                     : $1 eq 'lz' ? 'plzip'
                     : croak "Open: can't write to '$1' file $file";
            open $fh, "| $prog > $file"
            ...
            my $prog = ($1 eq 'gz' || $1 eq 'bgz') ? ($GzipOut || $Gzip)
                     : $1 eq 'lz' ? 'plzip'
                     : croak "Open: can't append to '$1' file $file";
            open $fh, "| $prog >> $file"
